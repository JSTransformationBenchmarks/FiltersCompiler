...
const builder = require('../main/builder.js'); // Mock log to hide error messages
...
describe('Test builder', () => {
  it('Works', async () => {
    ...
    expect(filterLines.length).toBe(23); // Common include
    ...
    expect(filterLines.includes('example.com#$#h1 { background-color: blue !important }')).toBeTruthy(); // Check replace version comment

    expect(filterLines.includes('! Version: 11.9090.19.19')).toBeFalsy();
    expect(filterLines.includes('! OriginalVersion: 11.9090.19.19')).toBeTruthy(); // Common_1 include

    expect(filterLines.includes('test-common-1-rule.com')).toBeTruthy(); // Exclude_1

    expect(filterLines.includes('test-common-1-rule.com$xmlhttprequest')).toBeFalsy(); // Strip comments

    expect(filterLines.includes('! some common rules could be places here, but comment are stripped')).toBeFalsy(); // Exclude
  ...
  it('Builds platforms', async () => {
    ...
    expect(filtersMetadata.filters[0].trustLevel).toBe('full'); // Obsolete Filter test
    ...
    const filtersI18nMetadataFilters = Object.keys(filtersI18nMetadata.filters); // Obsolete Filter test
    ...
    expect(filterLines.indexOf('~nigma.ru,google.com$$div[id=\"ad_text\"][wildcard=\"*teasernet*tararar*\"]') >= 0).toBeTruthy();
    expect(filterLines.indexOf('excluded_platform') >= 0).toBeTruthy();
    expect(filterLines.indexOf('test_domain#%#testScript();') >= 0).toBeTruthy(); // Check MAC platform
    ...
    expect(filtersMetadataMAC.groups).toBeTruthy();
    expect(filtersMetadataMAC.tags).toBe(undefined);
    expect(filtersMetadataMAC.filters).toBeTruthy(); // Check conditions
    ...
    expect(filterLines.indexOf('if_adguard_rule') >= 0).toBeTruthy(); // wrong condition

    expect(filterLines.indexOf('wrong_condition') >= 0).toBeFalsy(); // Check includes

    expect(filterLines.indexOf('!#include') >= 0).toBeFalsy(); // platform specify includes

    filterContent = await readFile(path.join(platforms, 'mac', 'filters', '4_optimized.txt'));
    expect(filterContent).toBeTruthy();
    filterLines = filterContent.split('\r\n');
    expect(filterLines.length).toEqual(13);
    expect(filterLines.indexOf('if_mac_included_rule') < 0).toBeTruthy(); // do not remove directives while stripped comment. `directives_not_stripped` rule should not remain

    expect(filterLines.indexOf('directives_not_stripped') >= 0).toBeFalsy(); // Check condition directives for platforms
    ...
    expect(filterLines.indexOf('||example.com/banner$image,redirect=1x1-transparent.gif') >= 0).toBeTruthy(); // Testing `platformsIncluded` directive
    // check if Directives Filter was built for test platform and metadata added to it's filters.json

    filterContent = await fs_existsPromise(path.join(platforms, 'test', 'filters', '4.txt'));
    expect(filterContent).toBeTruthy();
    filterContent = await fs_existsPromise(path.join(platforms, 'test', 'filters', '4_optimized.txt'));
    expect(filterContent).toBeTruthy();
    let metadata = await readFile(path.join(platforms, 'test', 'filters.json'));
    metadata = JSON.parse(metadata);
    expect(metadata).toBeTruthy();
    expect(metadata.filters.some(f => f.filterId === 4)).toBeTruthy(); // check if Directives Filter was built for mac platform and metadata added to it's filters.json

    filterContent = await fs_existsPromise(path.join(platforms, 'mac', 'filters', '4.txt'));
    expect(filterContent).toBeTruthy();
    filterContent = await fs_existsPromise(path.join(platforms, 'mac', 'filters', '4_optimized.txt'));
    expect(filterContent).toBeTruthy();
    metadata = await readFile(path.join(platforms, 'mac', 'filters.json'));
    metadata = JSON.parse(metadata);
    expect(metadata).toBeTruthy();
    expect(metadata.filters.some(f => f.filterId === 4)).toBeTruthy(); // check if Directives Filter was NOT built for ios platform and metadata was NOT added to it's filters.json

    filterContent = await fs_existsPromise(path.join(platforms, 'ios', 'filters', '4.txt'));
    expect(filterContent).toBeFalsy();
    filterContent = await fs_existsPromise(path.join(platforms, 'ios', 'filters', '4_optimized.txt'));
    expect(filterContent).toBeFalsy();
    metadata = await readFile(path.join(platforms, 'ios', 'filters.json'));
    metadata = JSON.parse(metadata);
    expect(metadata).toBeTruthy();
    expect(metadata.filters.some(f => f.filterId === 4)).toBeFalsy(); // check if Directives Filter was NOT built for test2 platform and metadata was NOT added to it's filters.json

    filterContent = await fs_existsPromise(path.join(platforms, 'test2', 'filters', '4.txt'));
    expect(filterContent).toBeFalsy();
    filterContent = await fs_existsPromise(path.join(platforms, 'test2', 'filters', '4_optimized.txt'));
    expect(filterContent).toBeFalsy();
    metadata = await readFile(path.join(platforms, 'test2', 'filters.json'));
    metadata = JSON.parse(metadata);
    expect(metadata).toBeTruthy();
    expect(metadata.filters.some(f => f.filterId === 4)).toBeFalsy(); // Testing `platformsExcluded` directive
    // check if Test Filter was NOT built for mac platform and metadata was NOT added to it's filters.json

    filterContent = await fs_existsPromise(path.join(platforms, 'mac', 'filters', '3.txt'));
    expect(filterContent).toBeFalsy();
    filterContent = await fs_existsPromise(path.join(platforms, 'mac', 'filters', '3_optimized.txt'));
    expect(filterContent).toBeFalsy();
    metadata = await readFile(path.join(platforms, 'mac', 'filters.json'));
    metadata = JSON.parse(metadata);
    expect(metadata).toBeTruthy();
    expect(metadata.filters.some(f => f.filterId === 3)).toBeFalsy(); // check if Test Filter was built for ios platform and metadata added to it's filters.json

    filterContent = await fs_existsPromise(path.join(platforms, 'ios', 'filters', '3.txt'));
    expect(filterContent).toBeTruthy();
    filterContent = await fs_existsPromise(path.join(platforms, 'ios', 'filters', '3_optimized.txt'));
    expect(filterContent).toBeTruthy();
    metadata = await readFile(path.join(platforms, 'ios', 'filters.json'));
    metadata = JSON.parse(metadata);
    expect(metadata).toBeTruthy();
    expect(metadata.filters.some(f => f.filterId === 3)).toBeTruthy(); // Testing `platformsIncluded` and `platformsExcluded` directive:
    // Test Platforms Filter should be built only for mac and chromium platforms only

    filterContent = await fs_existsPromise(path.join(platforms, 'mac', 'filters', '7.txt'));
    expect(filterContent).toBeTruthy();
    filterContent = await fs_existsPromise(path.join(platforms, 'mac', 'filters', '7_optimized.txt'));
    expect(filterContent).toBeTruthy();
    metadata = await readFile(path.join(platforms, 'mac', 'filters.json'));
    metadata = JSON.parse(metadata);
    expect(metadata).toBeTruthy();
    expect(metadata.filters.some(f => f.filterId === 7)).toBeTruthy();
    filterContent = await fs_existsPromise(path.join(platforms, 'test2', 'filters', '7.txt'));
    expect(filterContent).toBeTruthy();
    filterContent = await fs_existsPromise(path.join(platforms, 'test2', 'filters', '7_optimized.txt'));
    expect(filterContent).toBeTruthy();
    metadata = await readFile(path.join(platforms, 'test2', 'filters.json'));
    metadata = JSON.parse(metadata);
    expect(metadata).toBeTruthy();
    expect(metadata.filters.some(f => f.filterId === 7)).toBeTruthy();
    filterContent = await fs_existsPromise(path.join(platforms, 'ios', 'filters', '7.txt'));
    expect(filterContent).toBeFalsy();
    filterContent = await fs_existsPromise(path.join(platforms, 'ios', 'filters', '7_optimized.txt'));
    expect(filterContent).toBeFalsy();
    metadata = await readFile(path.join(platforms, 'ios', 'filters.json'));
    metadata = JSON.parse(metadata);
    expect(metadata).toBeTruthy();
    expect(metadata.filters.some(f => f.filterId === 7)).toBeFalsy();
    filterContent = await fs_existsPromise(path.join(platforms, 'test', 'filters', '7.txt'));
    expect(filterContent).toBeFalsy();
    filterContent = await fs_existsPromise(path.join(platforms, 'test', 'filters', '7_optimized.txt'));
    expect(filterContent).toBeFalsy();
