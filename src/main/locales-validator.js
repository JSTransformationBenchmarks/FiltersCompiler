...
module.exports = (async () => {
  ...
  const LOCALES_FILE_EXTENSION = '.json'; // each message key should consist of three parts
  ...
  const areValidMessagesKeys = (keys, id) => {
    ...
    const areValidKeys = !keys.find(key => {
      const keyNameParts = key.split('.');
      const propPrefix = id.slice(0, -1);
      const filterId = Number(keyNameParts[1]);
      return keyNameParts.length !== MESSAGE_KEY_NAME_PARTS_COUNT || keyNameParts[0] !== propPrefix || !Number.isInteger(filterId) || !(filterId > 0) || !LOCALES_DATA[id].required.includes(keyNameParts[2]);
    });
    return areValidKeys;
  };
  /**
  ...
  const prepareWarningDetails = obj => {
    const details = Object.keys(obj).reduce((acc, key) => {
  ...
  const prepareWarnings = warnings => {
    const output = warnings.reduce((acc, data) => {
  ...
  const validate = (dirPath, requiredLocales) => {
    ...
    const requiredFiles = Object.keys(LOCALES_DATA).map(el => `${el}${LOCALES_FILE_EXTENSION}`);
    locales.forEach(locale => {
      const localeWarnings = [];
      const filesList = readDir(path.join(dirPath, locale)); // checks all needed files presence

      const missedFiles = requiredFiles.filter(el => !filesList.includes(el));

      if (missedFiles.length !== 0) {
        localeWarnings.push([// if there are missedFiles, we consider it's critical
        WARNING_TYPES.CRITICAL, WARNING_REASONS.MISSED_FILES, missedFiles]);
      }

      const presentFiles = requiredFiles.filter(el => !missedFiles.includes(el)); // iterate over existent files
      ...
      presentFiles.forEach(fileName => {
        ...
        try {
          ...
          localeWarnings.push([// if there is invalid data format, we consider it's critical
          WARNING_TYPES.CRITICAL, WARNING_REASONS.NO_MESSAGES, [fileName]]);
          return;
        }
        ...
        if (messagesData.length === 0) {
          ...
          const warningType = requiredLocales.includes(locale) ? WARNING_TYPES.CRITICAL : WARNING_TYPES.LOW;
          localeWarnings.push([warningType, WARNING_REASONS.NO_MESSAGES, [fileName]]);
        }
        ...
        messagesData.forEach(obj => {
          ...
          if (!areValidMessagesKeys(messagesKeys, id) || !areValidMessagesValues(messagesValues)) {
            localeWarnings.push([// invalid messages data object is always critical
            WARNING_TYPES.CRITICAL, WARNING_REASONS.INVALID_DATA_OBJ, prepareWarningDetails(obj)]);
          }
        });
      });
    ...
    const isOK = !results.some(res => {
      const isCriticalWarning = res.warnings.some(warning => warning.type === WARNING_TYPES.CRITICAL);
